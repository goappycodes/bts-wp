...
<div class="invoice-greeting" style="line-height:2;margin:40px 0">
    <?php 
    if($language=='de'){
     _e ( 'Sehr geehrte/r', 'yith-woocommerce-pdf-invoice' );
    }
    else
    {
     _e ( 'Dear Mr/Mrs', 'yith-woocommerce-pdf-invoice' );   
    }
    ?> 
    <?php echo $first_name.' '.$last_name; ?>,<br/>
    <?php 
    if($language=='de'){
    _e ( 'vielen Dank, dass Sie sich für Brieftaubenshop.de entschieden haben.', 'yith-woocommerce-pdf-invoice' );   
    }
    else
    {
    _e ( 'Thank you for choosing Brieftaubenshop.de.', 'yith-woocommerce-pdf-invoice' ); 
    }
    ?>
    </div>
...
<table class="invoice-details">
	...
	<tr>
		...
		<?php if ( ywpi_is_enabled_column_quantity ( $document ) ) : ?>
			<th class="column-quantity" style="padding:0px 10px;text-align:center">
			    <?php 
			    if($language=='de'){
			    _e ( 'Menge', 'yith-woocommerce-pdf-invoice' );
			    }
			    else
			    {
			    _e ( 'Qty', 'yith-woocommerce-pdf-invoice' );
			    }
			    ?>
			</th>
		<?php endif; ?>
		...
		<th class="column-product">
		    <?php
		     if($language=='de'){
		     _e ( 'Artikel', 'yith-woocommerce-pdf-invoice' );    
		     }
		     else
		     {
		     _e ( 'Product', 'yith-woocommerce-pdf-invoice' );
		     }
		    ?>
		 </th>
	    ...
	    <th class="column-price">
	        <?php 
	        if($language=='de'){
	         _e ('Preis', 'yith-woocommerce-pdf-invoice' );    
	        }
	        else
	        {
	        _e ( 'Product price', 'yith-woocommerce-pdf-invoice' ); 
	        }
	        ?>
	   </th>
		...
		<?php if ( ywpi_is_enabled_column_product_price ( $document ) ) : ?>
			<th class="column-price">
			    <?php
			    if($language=='de'){
			    _e ( 'Netto', 'yith-woocommerce-pdf-invoice' );    
			    }
			    else
			    {
			    _e ( 'Base price', 'yith-woocommerce-pdf-invoice' );
			    }
			    ?>
			</th>
		<?php endif; ?>
		...
		<?php if ( ywpi_is_enabled_column_sale_price ( $document ) ) : ?>
			<th class="column-price" style="text-align:right"><?php _e ( 'Price', 'yith-woocommerce-pdf-invoice' ); ?></th>
		<?php endif; ?>
		...
		<?php if ( ywpi_is_enabled_column_line_total ( $document ) ) : ?>
			<th class="column-price" style="text-align:right"><?php _e ( 'Line total', 'yith-woocommerce-pdf-invoice' ); ?></th>
		<?php endif; ?>
		...
		<?php if ( ywpi_is_enabled_column_percentage_tax ( $document ) ) : ?>
			<th class="column-price">
			    <?php 
			    if($language=='de'){
			    _e ( 'MwSt (%)', 'yith-woocommerce-pdf-invoice' );    
			    }
			    else
			    {
			    _e ( 'Tax (%)', 'yith-woocommerce-pdf-invoice' );
			    }
			    ?>
			</th>
		<?php endif; ?>
		...
		<?php if ( ywpi_is_enabled_column_total_taxed ( $document ) ) : ?>
			<th class="column-price">
			    <?php
			    if($language=='de'){
			    _e ( 'Summe', 'yith-woocommerce-pdf-invoice' );     
			    }
			    else
			    {
			    _e ( 'Total', 'yith-woocommerce-pdf-invoice' ); 
			    }
			    ?>
			</th>
		<?php endif; ?>
	...
	foreach ( $invoice_details->get_order_items () as $item_id => $item ) {
		...
		<tr>
			...
			<?php if ( ywpi_is_enabled_column_quantity ( $document ) ) : ?>
				<td class="column-quantity" style="text-align:center">
			...
			<td class="column-product">
				...
				<small style="display:block; word-break: break-word; white-space: normal; max-width: 100px">
				<?php if ( ywpi_is_enabled_column_variation ( $document ) ) : ?>
					<?php echo wpautop(urldecode($invoice_details->get_variation_text ( $item_id, $_product ))) ?>
				<?php endif; ?>
				</small>
			...
			<td class="column-price">
			         <?php 
			          $_product_id = yit_get_prop ( $_product, 'id' );
			          $regular_price=get_post_meta($_product_id,'_regular_price',true);
			          $regular_price=wc_price($regular_price);
			          //$discount=$invoice_details->get_item_percentage_discount ( $item );
			          $discount=get_post_meta($current_order->id,'_cart_discount', true);
			          //$discount=0;
			          ...
			          //$price=round($invoice_details->get_item_price_per_unit ( $item ),2);
			          ...
			          $total_price=$price+$tax;   
			          /*}*/
			          $total_summe=round($total_price * $qty,2);
			          $t_price=$price*$qty;
			          
			          $main_price=wc_price($total_price);
			          
			          if($regular_price != $main_price){
			              // echo '<span class="sale-price">'.$regular_price.'</span>';
			          }
			...
			<?php if ( ywpi_is_enabled_column_product_price ( $document ) ) : ?>
				<td class="column-price">
					<?php echo $invoice_details->get_order_currency( $current_order, $price ); ?>
				</td>
			<?php endif; ?>
			...
			<?php if ( ywpi_is_enabled_column_percentage_tax ( $document ) && isset($item['line_tax']) && isset($item['line_total']) ) : ?>
				<td class="column-price">
				    <?php if(!empty($discount) && $discount > 0):?>
    					<?php if( $total_summe != 0 && $total_summe != '' ): ?>
    						<?php $tax_percentage = $item['line_tax']*100/$t_price; ?>
    						<?php echo round($tax_percentage) . '%'; ?>
    					<?php else: ?>
    						<?php echo '0%'; ?>
    					<?php endif; ?>
    				<?php else: ?>
			...
			<?php if ( ywpi_is_enabled_column_total_taxed ( $document ) ) : ?>
				<td class="column-price">
				    <?php if(!empty($discount) && $discount > 0):?>
					        <?php echo $invoice_details->get_order_currency( $current_order, $total_summe ); ?>
					<?php else: ?>
	...
	if ( apply_filters ( 'ywpi_is_visible_fee_details_section', true, $document ) ) :
		...
		foreach ( $invoice_details->get_order_fees () as $item_id => $item ) {
			...
			<tr class="border-top">
                ...
                <?php
				$fee_amount=$item['line_total'];
				$fee_tax=$item['line_tax'];
				$total_amount=$fee_amount+$fee_tax;
				?>
				<td class="column-price">
						<?php echo $invoice_details->get_order_currency( $current_order, $total_amount ); ?>
				</td>
